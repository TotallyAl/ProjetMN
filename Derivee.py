...
# Fonction générale pour les variations de certains paramètres


def odefunction_g(var: float, C: list, param: str, captage: bool) -> np.array:
    """
    # Odefunction_g
    ...
    ## Paramètres
        -> var: Une variable qui va varier lorsqu'elle sera résolue dans `scipy.integrate.solve_ivp()`
        -> C: Une liste contenant les concentrations des composés, X, T et P présent dans le système
        -> param: C'est le paramètre qui indique le paramètre qui varie
            -> "z": La position dans le réacteur (n'a pas un impact sur les calculs)
            -> "u_g": La vitesse du gaz (a un impact sur les calculs)
            -> "u_s": La vitesse du solide (a un impact sur les calculs)
        -> captage: Un booléen qui indique si on veut capturer le CO2 ou non
    """
    # Valeur par defaut si les parametres ne varient pas
    u_g = Constantes.u_g
    u_s = Constantes.u_s

    # Ce code va generaliser les variations qu'on pourra faire a la fonction
    if param == "z":
        z = var
        """Operation inutile...."""
    elif param == "u_g":
        ...
        raise ValueError("Parametre invalide dans odefunction_g()")
    ...
    dX_dz: float = (Constantes.M_CaO / u_s) * r_CBN
    ...
    Re_p: float = (u_g * Constantes.epsilon * rho_g * Constantes.d_p) / Constantes.mu
    ...
    DENOM: float = (
        1 - Constantes.epsilon
    ) * rho_s * u_s * Constantes.C_ps + rho_g * u_g * Constantes.C_pg
    ...
    dP_dz: float = -(
        (rho_g * (u_g**2) / Constantes.d_p)
        * ((1 - Constantes.epsilon) / Constantes.epsilon)
        * (
            (150 * (1 - Constantes.epsilon) * Constantes.mu)
            / (Constantes.d_p * rho_g * u_g)
            + 1.75
        )
        * (10 ** (-5))
    )
...
def odefunction_q(z: float, C: list, u_g: float, u_s: float) -> np.array:
